...
/*   Updated: 2017/06/14 16:52:19 by vafanass         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		texture_floor(t_wolf *wolf)
{
	...
	while (++wolf->y < wolf->h)
	{
		...
		wolf->weight = (wolf->currentdist - wolf->distplayer) /
			(wolf->distfromwall - wolf->distplayer);
		wolf->currentfloorx = wolf->weight * wolf->floorxwall +
			(1.0 - wolf->weight) * wolf->posx;
		wolf->currentfloory = wolf->weight * wolf->floorywall +
			(1.0 - wolf->weight) * wolf->posy;
		wolf->floortexx = (int)(wolf->currentfloorx * TEXT_W) % TEXT_W;
		wolf->floortexy = (int)(wolf->currentfloory * TEXT_H) % TEXT_H;
		...
		esdl_read_pixel(wolf->ceiling_texture, wolf->floortexx,
			wolf->floortexy));
	}
}
