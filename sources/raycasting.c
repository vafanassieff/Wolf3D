...
/*   Updated: 2017/06/14 17:04:05 by vafanass         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_raycasting(t_wolf *wolf)
{
	...
	wolf->deltadistx = sqrt(1 + (wolf->raydiry * wolf->raydiry) /
		(wolf->raydirx * wolf->raydirx));
	wolf->deltadisty = sqrt(1 + (wolf->raydirx * wolf->raydirx) /
...
static void	texture_wall(t_wolf *wolf)
{
	...
	while (++wolf->y < wolf->drawend)
	{
		...
		wolf->texy = ((wolf->d * TEXT_H) / wolf->lineheight) / 256;
		esdl_put_pixel(wolf->surf, wolf->x, wolf->y,
		esdl_read_pixel(wolf->wall_texture[wolf->textnb], wolf->texx,
			wolf->texy));
	}
}
...
{
	wolf->x = -1;
	while (++wolf->x < wolf->w + 1)
	{
		init_raycasting(wolf);
		find_step(wolf);
		dda(wolf);
		if (wolf->side == 0)
			wolf->walldist = (wolf->mapx - wolf->rayposx +
				(1 - wolf->stepx) / 2) / wolf->raydirx;
		else
			wolf->walldist = (wolf->mapy - wolf->rayposy +
